...
def build_svn_repo():
    ...
    commits = [
        ("r1: scaffold main.py + config (heavy AI by dev0)", {
            "main.py": "from config import DEBUG\ndef run():\n    pass\nif __name__ == '__main__':\n    run()\n",
            "config.py": "DEBUG = True\nAPI_VERSION = 'v1'\n",
        }, 85),
        ("r2: add auth module (AI generated by dev2)", {
            "auth.py": "def register(email, pw):\n    return True\n\ndef login(email, pw):\n    return True\n",
        }, 75),
        ("r3: add utils + DB layer (mixed AI-human by dev4)", {
            "utils.py": "def hash_pw(pw):\n    import hashlib\n    return hashlib.sha256(pw.encode()).hexdigest()\n",
            "db.py": "def connect():\n    pass\n\ndef execute(sql):\n    pass\n",
        }, 50),
        ("r4: human review — tighten auth security (dev8)", {
            "auth.py": "def register(email, pw):\n    if '@' not in email:\n        raise ValueError('bad email')\n    return True\n\ndef login(email, pw):\n    return True\n",
        }, 10),
        ("r5: AI refactor — add models (dev1)", {
            "models.py": "class User:\n    def __init__(self, email):\n        self.email = email\n    def to_dict(self):\n        return {'email': self.email}\n",
        }, 80),
        ("r6: human polish — connect wiring (dev9)", {
